...
let bitapRegexSearch = __webpack_require__(5);
let bitapSearch = __webpack_require__(7);
let patternAlphabet = __webpack_require__(4);

let Bitap = function () {
  function Bitap(pattern, _ref) {
    let _ref$location = _ref.location,
...
let isArray = __webpack_require__(0);

let deepValue = function deepValue(obj, path, list) {
  if (!path) {
...
let bitapScore = __webpack_require__(6);
let matchedIndices = __webpack_require__(3);
...
module.exports = function (text, pattern, patternAlphabet, _ref) {
  ...
  for (let _i = 0; _i < patternLen; _i += 1) {
    ...
    while (binMin < binMid) {
      let _score3 = bitapScore(pattern, {
        errors: _i,
    ...
    let _score2 = bitapScore(pattern, {
      errors: _i + 1,
...
let Bitap = __webpack_require__(1);
let deepValue = __webpack_require__(2);
let isArray = __webpack_require__(0);
...
let Fuse = function () {
  ...
  _createClass(Fuse, [{
    ...
    value: function search(pattern) {
      let opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : { limit: false };

      this._log('---------\nSearch pattern: "' + pattern + '"');

      let _prepareSearchers2 = this._prepareSearchers(pattern),
          tokenSearchers = _prepareSearchers2.tokenSearchers,
          fullSearcher = _prepareSearchers2.fullSearcher;

      let _search2 = this._search(tokenSearchers, fullSearcher),
    ...
    value: function _computeScore(weights, results) {
      ...
      for (let i = 0, len = results.length; i < len; i += 1) {
        let output = results[i].output;
        let scoreLen = output.length;
